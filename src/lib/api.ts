...
export const getEventTypeStats = async () => {
  const response = await fetch(`${API_BASE_URL}/api/feedback/stats/event-types`);
...
export const getTrendStats = async (timeRange: string) => {
  const response = await fetch(`${API_BASE_URL}/api/feedback/stats/trends?timeRange=${timeRange}`);
